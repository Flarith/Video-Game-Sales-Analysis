...
# --- Upload de CSV ---
uploaded_file = st.file_uploader("Faça upload do CSV com novos jogos", type=["csv"])
...
if uploaded_file:
    ...
    st.markdown('<h3 style="color:white;">Indicadores Principais</h3>', unsafe_allow_html=True)
    col1, col2, col3 = st.columns(3)
    col1.metric(f"Total de Vendas ({', '.join(regioes_sel)})", round(vendas_total, 2))
    col2.metric("Maior Venda", round(filtrado['Global_Sales'].max(), 2))
    col3.metric("Quantidade de Jogos", len(filtrado))

    # --- Gráficos organizados em abas ---
    tab1, tab2, tab3 = st.tabs([" Vendas por Ano", " Top Jogos", " Plataformas & Distribuições"])

    with tab1:
        # vendas totais por ano
        if 'Year' in filtrado.columns:
            vendas_ano = filtrado.groupby('Year')['Global_Sales'].sum().reset_index()
            fig_ano = px.line(
                vendas_ano, x='Year', y='Global_Sales',
        ...
        # vendas por ano por regiões selecionadas
        if len(regioes_sel) > 0:
            vendas_ano_regiao = filtrado.groupby('Year')[regioes_sel].sum().reset_index()
            fig_regiao = px.bar(
                vendas_ano_regiao, x='Year', y=regioes_sel,
                title="Vendas por Ano por Região",
                labels={'value':'Vendas','Year':'Ano'},
                text_auto=True
            )
            st.plotly_chart(fig_regiao, use_container_width=True)

    with tab2:
        # top 10 jogos com maiores vendas
        if 'Name' in filtrado.columns:
            top_jogos = filtrado.sort_values(by='Global_Sales', ascending=False).head(10)
            fig_top = px.bar(
                top_jogos, x='Name', y='Global_Sales',
                title="Top 10 Jogos com Maiores Vendas",
            ...
            # Top jogos por gênero
            fig_treemap = px.treemap(
                top_jogos, path=['Genre','Name'], values='Global_Sales',
    ...
    with tab3:
        ...
        # Donut chart de regiões filtradas
        if len(regioes_sel) > 0:
            vendas_paises = pd.DataFrame({
                'Região': regioes_sel,
                'Vendas': [filtrado[r].sum() for r in regioes_sel]
            })
            fig_donut = px.pie(
                vendas_paises, names='Região', values='Vendas',
        ...
        # Donut chart de gêneros
        if 'Genre' in filtrado.columns:
            vendas_genero = filtrado.groupby('Genre')['Global_Sales'].sum().reset_index()
            fig_genero = px.pie(
                vendas_genero, names='Genre', values='Global_Sales',
